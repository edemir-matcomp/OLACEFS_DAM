...
def initialize_model(model_name, num_classes, channels, feature_extract, use_pretrained):
    ...
    if model_name == "resnet":
        ...
        #= torch.nn.Conv2d(13, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        ...
        model_ft.features[0] = nn.Conv2d(len(channels), 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        set_parameter_requires_grad(model_ft, feature_extract)
        num_ftrs = model_ft.classifier[6].in_features
        model_ft.classifier[6] = nn.Linear(num_ftrs,num_classes)
        input_size = 224
